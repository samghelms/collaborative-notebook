...
const handleDrag = (e, anchor, nextRows, pos, i, j, rowId) => {
...
export default class BreadLoaf extends React.Component {
    ...
    append(item){
        ...
        const rowId = this.makeRowID()
        this.updateLayout(this.getLayout().concat([{
            rowId: rowId,
            items: [ item ]
        }]), 'append', item, {rowId: rowId})
    }
    prepend(item){
        if(!item.id) item.id = uuid();
        const rowId = this.makeRowID()
        this.updateLayout([{
            rowId: rowId,
            items: [ item ]
        }].concat(this.getLayout()), 'prepend', item, {rowId: rowId})
    }
    ...
    endDrag(e){
        if(this.state.dockTarget && this.state.dragObject){
            ...
            var rowId = this.makeRowID()
            let [nextRows, oldThing] = handleDrag(e, anchor, nextRows, pos, i, j, rowId)
            
            this.updateLayout(nextRows, 'drag', oldThing, {e, pos, thing, nextRows, anchor, i, j, rowId})
        }
    ...
    makeElement(data, pos){
        return React.cloneElement(this.props.element, {
            ...
            updateView: d => {
                ...
                newRows[rowi].items[coli] = {...data, ...d}
                this.updateLayout(newRows, 'update', data, {rowi: rowi, coli: coli, d: d})
            },
            close: (...args) => {
                var newRows = this.cloneLayout()
                let [rowi, coli] = locateKey(newRows, data.id)
                newRows[rowi].items.splice(coli, 1)
                this.updateLayout(newRows, 'close', data, {rowi: rowi, coli: coli})
            },
            fork: (...args) => {
                var newRows = this.cloneLayout()
                let [rowi, coli] = locateKey(newRows, data.id)
                ...
                this.updateLayout(newRows, 'fork', newData, {rowi: rowi, coli: coli + 1})
            },
            after: (...args) => {
                var newRows = this.cloneLayout()
                let [rowi, coli] = locateKey(newRows, data.id)
                let rowId = this.makeRowID()
                let newData = this.makeSlice() //Object.assign({}, data, { id: this.makeSliceID() });
                newRows.splice(rowi+1, 0, { rowId, items: [ newData ] })
                this.updateLayout(newRows, 'insert', newData, {rowi: rowi+1, rowId})
            },
            before: (...args) => {
                var newRows = this.cloneLayout()
                let [rowi, coli] = locateKey(newRows, data.id)
                let rowId = this.makeRowID()
                let newData = this.makeSlice() //Object.assign({}, data, { id: this.makeSliceID() });
                newRows.splice(rowi, 0, { rowId, items: [ newData ] })
                this.updateLayout(newRows, 'insert', newData, {rowi, rowId})
            },
            beginDrag: e => {
        ...
        <FlipMove onStart={(childElement, domNode) => {
            ...
            <div className={"bread-row " + (
                ...
                <div className="divider divider-top" onClick={d => {
                    ...
                    let rowId = this.makeRowID()
                    newRows.splice(rowi, 0, { rowId, items: [  item ] })
                    this.updateLayout(newRows, 'insert', item, {rowi, rowId})
                }} />
            </div>
            ...
            {this.getLayout().map((row, rowi) => 
                <div className={"bread-row real-bread-row " + (
                        ...
                        {row.items.map((data, coli) => 
                            ...
                            <div className={"bread-col " + (
                                ...
                                { coli == 0 && <div className="vertical-divider divider-left" onClick={d => {
                                    ...
                                    this.updateLayout(newRows, d.altKey ? 'fork' : 'insert-item', item, {rowi, coli})
                                }} /> }
                                ...
                                <div className="vertical-divider divider-right" onClick={d => {
                                    ...
                                    this.updateLayout(newRows, d.altKey ? 'fork' : 'insert-item', item, {rowi, coli: coli + 1})
                                }} />
                            </div>
                    ...
                    <div className="divider divider-bottom"  onClick={d => {
                        ...
                        let item = Object.assign({}, d.altKey ? data : this.makeSlice(), { id: this.makeSliceID() })
                        let rowId = this.makeRowID()
                        newRows.splice(rowi + 1, 0, { rowId, items: [  item ] })
                        this.updateLayout(newRows, 'insert', item, {rowi: rowi + 1, rowId})
                    }} />
                </div>
            )}
